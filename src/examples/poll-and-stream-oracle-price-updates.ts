...
async function fetchAndUpdateOraclePriceData(cache: CustodyToOraclePrice) {
    ...
    DOVES_ORACLES.forEach(({ custody }, index) => {
        ...
        if (!feed) {
            throw new Error(`Failed to fetch latest oracle price data for: ${custody.toString()}`);
        }
...
export async function subscribeOraclePrices(intervalMs: number = 100) {
    ...
    connection.onProgramAccountChange(
        ...
        ({ accountId, accountInfo }) => {
            const oracle = DOVES_ORACLES.find((oracle) => oracle.publicKey.equals(accountId));

            if (!oracle) {
                throw new Error(`Cannot find custody details for account: ${accountId.toString()}`);
            }

            const priceFeed = dovesProgram.coder.accounts.decode("priceFeed", accountInfo.data);

            const data: DovesOraclePrice = {
                price: priceFeed.price,
                priceUsd: BNToUSDRepresentation(priceFeed.price, Math.abs(priceFeed.expo)),
